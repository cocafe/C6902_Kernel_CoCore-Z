				       ...
				       struct mdss_panel_info *panel_info)
{
	...
	vporch = panel_info->lcdc.v_back_porch +
	    panel_info->lcdc.v_front_porch + panel_info->lcdc.v_pulse_width;
	panel_info->te.sync_cfg_height = (!rc ? tmp : ((panel_info->yres + vporch) * 2));
	rc = of_property_read_u32
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if (spec_pdata->cabc_early_on_v3_cmds.cmd_cnt &&
		...
		mdss_dsi_panel_cmds_send(ctrl_pdata, &spec_pdata->cabc_early_on_v3_cmds);
		spec_pdata->cabc_active = 1;
	}
...
static void cabc_work_fn(struct work_struct *work)
{
	...
	if (spec_pdata->cabc_deferred_on_cmds.cmd_cnt &&
		...
		mdss_dsi_panel_cmds_send(ctrl_pdata, &spec_pdata->cabc_deferred_on_cmds);
		spec_pdata->cabc_active = 1;
	...
	if (spec_pdata->cabc_deferred_on_v3_cmds.cmd_cnt &&
		...
		mdss_dsi_panel_cmds_send(ctrl_pdata, &spec_pdata->cabc_deferred_on_v3_cmds);
		spec_pdata->cabc_active = 1;
...
static int mdss_dsi_panel_disp_on(struct mdss_panel_data *pdata)
{
	...
	if ((spec_pdata->cabc_deferred_on_cmds.cmd_cnt ||
				spec_pdata->cabc_deferred_on_cmds.cmd_cnt)  &&
				...
				int driver_ic, char *id_data)
{
	...
	for_each_child_of_node(parent, next) {
		...
		mdss_dsi_parse_dcs_cmds(next,
				...
				"somc,mdss-dsi-cabc-deferred-on-v3-command",
				NULL);
